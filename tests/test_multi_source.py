"""multi_source_shortest_paths: parallel rows == sequential single-source."""

import numpy as np
import pytest
import scipy.sparse as sp

from logtwothirds import multi_source_shortest_paths, shortest_paths
...
def test_rows_match_single_source(method):
    ...
    dist, pred = multi_source_shortest_paths(g, sources, method=method)
...
def test_empty_sources():
    ...
    dist, pred = multi_source_shortest_paths(g, [])
...
def test_source_out_of_range(method):
    ...
    with pytest.raises(IndexError):
        multi_source_shortest_paths(g, [0, 10], method=method)
    with pytest.raises(IndexError):
        multi_source_shortest_paths(g, [-1], method=method)
...
def test_negative_weight_rejected():
    ...
    with pytest.raises(ValueError):
        multi_source_shortest_paths((indptr, indices, weights), [0])


def test_unknown_method():
    g = _random_csr(10, 30, seed=3)
    with pytest.raises(ValueError, match="unknown method"):
        multi_source_shortest_paths(g, [0], method="bfs")
