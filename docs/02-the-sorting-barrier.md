...
order of your numbers. So Dijkstra — any algorithm that settles vertices in
distance order — contains a sorting algorithm inside it.
...
Therefore any shortest-path algorithm that works like Dijkstra — settling
vertices from nearest to farthest — is stuck paying `n log n`. On sparse
...
*(Fine print, for honesty: this cost model — comparisons and additions on
real-valued weights — is the standard one for the problem, and it's the model
