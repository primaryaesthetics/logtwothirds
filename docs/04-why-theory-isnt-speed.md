...
The gap does narrow as `n` grows (84×, 45×, 29×, 26× at n = 10⁴ through 10⁷), in the direction, and roughly at the rate, the `log^(1/3) n` scaling
...
ties below). That's close enough to touch, and ahead of every published
...
innocent-looking rule in the paper (relax on *less-than-or-equal*, not just less-than) turns out to interact with equal-length paths so that a naive
